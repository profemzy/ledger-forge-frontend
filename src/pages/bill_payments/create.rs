...
use crate::api::{bill_payments as api_bp, contacts as contacts_api, accounts as accounts_api, bills as bills_api};
use crate::types::bill_payments::{CreateBillPaymentRequest, BillPaymentApplicationRequest};
use crate::types::contacts::Contact;
use crate::types::accounts::{Account, AccountType};
use crate::types::bills::Bill;
use crate::state::{ToastContext, ToastKind};
use crate::utils::format::{format_money, mask_money_input};

#[component]
pub fn BillPaymentCreate() -> impl IntoView {
    ...
    let (bank_account, set_bank_account) = create_signal(None::<Uuid>);
    ...
    let bank_accounts_res = create_resource(|| (), |_| async move { accounts_api::list_accounts(Some(AccountType::Asset), None, Some(false)).await });

    let bills_res = create_resource(move || vendor_id.get(), |vid| async move {
        if let Some(v) = vid { bills_api::list_bills(Some(v), None, Some(100)).await } else { Ok(Vec::<Bill>::new()) }
    });

    use std::collections::HashMap;
    let (apps, set_apps) = create_signal::<HashMap<Uuid, String>>(HashMap::new());

    let total_applied = move || apps.get().values().filter_map(|s| s.replace(",", "").parse::<Decimal>().ok()).fold(Decimal::ZERO, |a,b| a+b);
    ...
    let on_submit = create_action(move |_: &()| {
        ...
        async move {
            set_error.set(None);
            let Some(vendor_id) = vid else { set_error.set(Some("Vendor is required".into())); return; };
            ...
            let mut applications: Vec<BillPaymentApplicationRequest> = Vec::new();
            for (bill_id, val) in apps_map.into_iter() {
                let v = val.replace(",", "").parse::<Decimal>().unwrap_or(Decimal::ZERO);
                if v > Decimal::ZERO { applications.push(BillPaymentApplicationRequest { bill_id, amount_applied: v }); }
            }

            let req = CreateBillPaymentRequest {
            ...
            match api_bp::create_bill_payment(&req).await {
                Ok(_) => { if let Some(t) = toaster { t.push("Bill payment recorded", ToastKind::Success); } navigate("/bills", Default::default()); }
    ...
    view! {
        <div class="p-6 max-w-5xl">
            <h1 class="text-2xl font-semibold mb-4">"New Bill Payment"</h1>
            ...
            <form on:submit=move |ev| { ev.prevent_default(); on_submit.dispatch(()); }>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div>
                        ...
                        <input class="w-full border rounded px-3 py-2" type="text" prop:value=move || payment_number.get() on:input=move |e| set_payment_number.set(event_target_value(&e)) />
                    </div>
                    <div>
                        ...
                        <Transition fallback=move || view!{ <div class="text-sm text-gray-500">"Loading..."</div> }>
                            {move || match vendors_res.get() {
                                Some(Ok(list)) => view!{
                                    <select class="w-full border rounded px-3 py-2" on:change=move |e| { let v = event_target_value(&e); if let Ok(id) = Uuid::parse_str(&v) { set_vendor_id.set(Some(id)); set_apps.set(HashMap::new()); } }>
                    ...
                    <div>
                        ...
                        <input class="w-full border rounded px-3 py-2" type="date" prop:value=move || payment_date_str.get() on:input=move |e| set_payment_date_str.set(event_target_value(&e)) />
                    </div>
                    <div>
                        ...
                        <input class="w-full border rounded px-3 py-2" type="text" inputmode="decimal" placeholder="0.00"
                            ...
                            on:input=move |e| set_amount_str.set(mask_money_input(&event_target_value(&e)))
                        />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div>
                        ...
                        <input class="w-full border rounded px-3 py-2" type="text" placeholder="Check / Cash / ACH"
                    ...
                    <div>
                        ...
                        <Transition fallback=move || view!{ <div class="text-sm text-gray-500">"Loading..."</div> }>
                            {move || match bank_accounts_res.get() {
                                Some(Ok(list)) => view!{
                                    <select class="w-full border rounded px-3 py-2" on:change=move |e| { let v = event_target_value(&e); if v.is_empty() { set_bank_account.set(None); } else if let Ok(id) = Uuid::parse_str(&v) { set_bank_account.set(Some(id)); } }>
                ...
                <h2 class="text-lg font-semibold mb-2">"Apply to Bills"</h2>
                <div class="overflow-auto">
                    <Transition fallback=move || view!{ <div>"Loading bills..."</div> }>
                        {move || match bills_res.get() {
                            Some(Ok(list)) => {
                                let open_bills: Vec<Bill> = list.into_iter().filter(|b| b.balance > Decimal::ZERO && !matches!(b.status, crate::types::bills::BillStatus::Paid)).collect();
                                if open_bills.is_empty() {
                                    view!{ <div class="text-gray-600">"No open bills for this vendor."</div> }.into_view()
                                } else {
                                    view!{
                                        <table class="w-full border-collapse bg-white dark:bg-gray-900 rounded shadow">
                                            ...
                                            <tbody>
                                                {open_bills.into_iter().map(|b| {
                                                    view!{
                                                        <tr class="border-b">
                                                            <td class="py-2 px-3">{b.bill_number.clone().unwrap_or_else(|| b.id.to_string())}</td>
                                                            ...
                                                            <td class="py-2 px-3">
                                                                <input class="border rounded px-2 py-1 w-32" type="text" inputmode="decimal" placeholder="0.00"
                                                                    on:input=move |e| {
                                                                        ...
                                                                        set_apps.update(|m| { m.insert(b.id, v); });
                                                                    }
                                                                />
                                                            </td>
                                                        </tr>
                ...
                <div class="mt-6">
                    <button class="bg-blue-600 text-white px-4 py-2 rounded" type="submit">"Record Bill Payment"</button>
                </div>
            </form>
        </div>
    }
